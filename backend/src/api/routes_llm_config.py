...
_encrypt_key = os.environ.get("ENCRYPT_KEY", "")
if not _encrypt_key:
    _encrypt_key = EncryptionManager.generate_key()
_mgr = EncryptionManager(_encrypt_key)
...
def encrypt_api_key(api_key: Optional[str]) -> Optional[str]:
    if not api_key:
        return None
    return _mgr.encrypt(api_key)


def decrypt_api_key(encrypted: Optional[str]) -> Optional[str]:
    if not encrypted:
        return None
    return _mgr.decrypt(encrypted)
